...
### Fixed
- `tk sync` no longer rebases and autostashes a repository it does not own. `git pull --rebase --autostash` acts on the repository's current branch and whole worktree — `-C <storeRoot>` only sets the working directory — so for a central store root nested inside another repo, the routine case of another machine having pushed first made sync stash the enclosing repo owner's entire uncommitted worktree, rebase their current branch onto its upstream and pop the stash, on `tk serve`'s unattended five-second loop. The rebase pull is now refused whenever the store root is not the repository's own toplevel, at the one function both the pre-flight pull and the post-push-rejection retry go through, and the store blocks with a `.tk-sync-blocked` marker naming the repository — so `tk status` reports it as `blocked:` and the serve log carries it rather than tickets quietly failing to propagate. The marker asks for the divergence to be reconciled by hand only when the branch really is behind its upstream, since the retry the gate sits behind runs on any failed push and not only a rejected one — a branch with no upstream reaches it too — and it now carries the push's own error, which was previously discarded, so the reason the push failed still reaches the user when no rebase would have helped; that output is written by the remote and its hooks, so it is filtered before it lands in the marker like every other git output sync records. `git push` is unchanged for a nested store: it publishes commits already on the owner's branch to their own upstream and rewrites neither, so outbound sync keeps working in the common non-diverged case. A comparison tk cannot resolve — no toplevel, or an unresolvable symlink on either side — counts as nested and refuses, the same unknown-is-not-none split the in-progress-operation guard makes. A store that is its own repository is untouched and still rebases.
- Every git output `tk sync` interpolates into a blocked-marker or warning message — fetch, add, commit, pull and push alike — now goes through the same control-character rule the rest of tk uses; every one of them went in raw. That output carries the remote's own transport and hook text, and git's messages name paths from the repository, whose ticket filenames arrive over git from other machines. The marker is a file `tk status` prints back on every later run and `tk serve` copies into its log, so a raw escape or newline could forge status and log lines and repaint the operator's terminal long after the cycle that wrote it. Each output is filtered where its message is built, which is also where the same text is returned as the cycle's warning, so the marker and the warning cannot disagree.
- An autostash pop conflict sync itself caused outside the store's own paths is no longer silent. `pull --rebase --autostash` exits 0 even when the pop conflicts, and the blocking unmerged-paths guard is scoped to `tickets/` and `config.yaml` on purpose — a conflict sync neither created nor can resolve must not stop the store syncing — which left a pop in the repo owner's files with no marker and no log line anywhere. After a successful pull, sync now scans the whole repository for unmerged paths and logs one line naming those outside its own paths, sanitized like every other path tk prints since a filename arrives over git from other machines. The scan pins `core.quotePath=false` and `-z` the way the staged-conflict-marker guard does: here the path decides whether it counts as inside the store rather than only being printed, and a non-ASCII or control-character name that came back C-quoted would fail that test and be reported as outside the store. One line carries at most ten names and a `+N more` tail, since unlike the store-scoped guard beside it this scan is repo-wide. It reports what it observes rather than what caused it: a stash pop the repo's owner left behind has no MERGE_HEAD either, so it is indistinguishable here from one the pull just made. It does not block: the store's own syncing continues, and paths inside the store stay the existing scoped guard's job.
