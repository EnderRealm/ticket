...
- `{command}` — the slash command the session opens on: `/work <id>` for `w`, `/capture <idea>` for `c`. The idea is typed into the TUI and sanitized exactly like `{title}`, so `{command}` takes the same quoting a title does. A template with no `{command}` — one written before it existed, hard-coding `/work {id}` — still serves `w` unchanged, since the placeholder simply never fires, and refuses `c` on the status line, since it has nowhere to put the capture
...
| `c` | Prompt for a one-line idea, then spawn a `/capture <idea>` session in a new terminal — in the board's project checkout from the list, the ticket's own from a detail — where Claude drafts the ticket; `esc` cancels. Same `spawn_command` and the same refusals as `w`, plus a template with no `{command}` |
