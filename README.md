...
checkout /Users/you/code/myproject; acceptance sha256:9f2c…
- PASS (exit 0): Frontier excludes blocked tickets.
- UNVERIFIED: The TUI redraws cleanly at 40 columns.
```

The second line is the run's provenance: the directory the commands actually ran in, and the identity of the acceptance contract they ran against — a sha256 over the parsed criteria (text, command, unverifiable claim and reason, in order), so an edit to what a criterion says or runs moves it and whitespace outside the criteria does not. A run started with a `candidate` (MCP only, below) adds `; candidate <name>` to that line. The record says which contract was verified and where; it does not say anyone approved the result.

`tk verify --json` and a completed `ticket_verify` MCP call return the same structured report, including each command's exit code and captured output (capped at 4KB per criterion), the actual `dir`, the `acceptance_id` and, when supplied, the `candidate`. The MCP tool executes commands on the server host in the checkout registered for the ticket's project, or in a linked worktree of it when asked.

#### Reading the contract and verifying a worktree over MCP

`ticket_criteria` returns a ticket's acceptance criteria exactly as `ticket_verify` parses them — `index` (1-based, the same index `tk verify --criterion` uses), `text`, `command` if any, `unverifiable` and `unverifiable_reason` — with `acceptance_id`, the identity of that contract. It runs nothing, and a ticket with no criteria returns an empty list rather than an error. A harness that intends to run or grade criteria itself reads them here instead of parsing the section a second time, so the index it grades is the one tk runs.

`ticket_verify` takes three optional arguments beside `id`:

- `dir` runs the commands in that directory instead of the configured checkout. It is accepted only if it is the configured checkout itself or the root of a linked git worktree sharing that checkout's repository (its `git rev-parse --git-common-dir`), judged on the resolved path. A subdirectory, an unrelated or nested repository, a directory that is not a git worktree, a missing path, an empty string, and a symlink alias resolving to any of those are refused before anything runs, naming why, and nothing is recorded. The report and the record name the resolved path. `dir` moves an eligible run; it does not make one eligible: Root, an unregistered project and a missing checkout are refused as before, and the allow-list and timeout are still the ticket's project's, from machine-local config.
- `acceptance_id` is the identity `ticket_criteria` returned. If the ticket's criteria no longer produce it, the run is refused before any directory is resolved or command starts, naming the identity you read and the current one. Whether or not it was supplied, a run whose criteria change while its commands are running is not recorded: the result comes back with `record_error` saying the contract changed, and the ticket's Test Results are left as they were, so a record never reads as evidence for criteria no command was run against.
- `candidate` is your name for what was under test — a commit, a worktree, a run ID — recorded as supplied (control characters stripped, capped at 256 characters) in the report and the record. It is provenance only.

Calls with `id` alone behave exactly as before: the configured checkout, the quick report shape, the record — now carrying the provenance line — and every refusal. The mid-run rule holds for `tk verify` too: it records through the same path, and a run whose criteria changed while its command ran reports the refusal as the record warning (or `record_error` under `--json`) and leaves the ticket unwritten.
...
A client request timeout or cancellation does not cancel the commands. If the start response was lost, recover this session's latest job with `ticket_verify_status` and the ticket `id`. Repeating `ticket_verify` while that ticket is active joins the job only when it asks for the same verification — the same `dir`, `acceptance_id` and `candidate` as the run in flight (an `id`-only repeat of an `id`-only start qualifies); a repeat that differs in any of the three, including one whose criteria changed since the run started, is refused naming the active run's directory, contract and candidate, and starts nothing. After the job finishes, a new `ticket_verify` intentionally starts another run. Use status for retrieval. Another session cannot join, inspect or cancel the job, or start the same ticket while it is active in this server.
...
`--criterion` and `--no-record` are CLI-only, and the `ticket_verify` MCP tool's `dir` is narrower than `--dir`: an MCP caller's arguments are shaped by ticket content, and a sandboxed client with no shell of its own must not gain reach it does not otherwise have, so the tool accepts only the configured checkout or a linked worktree of that same repository and refuses everything else (see above). A CLI caller already has a shell and can run anything in any directory, so `--dir` is used as given and widens nothing for it. The MCP tool always records. `verify_allow` is still read from `~/.ticket/config.yaml` alone — no flag or argument widens it, neither `--dir` nor `dir` relocates where it is read from, and `TK_STORE_ROOT` does not move it either.
...
A scheduled observer — a check that runs against a ticket's work after the fact and files what it finds — has to attach evidence, reopen a ticket or file a discovery without duplicating the work on a retry or overwriting a decision made in between. Separate `ticket_show`, `ticket_add_note` and `ticket_edit` calls cannot promise either across a lost response or an intervening writer, so tk exposes a narrow action protocol over MCP (`ticket_apply_action`, `ticket_discover`) and the library (`ticket.ApplyAction`, `ticket.CreateDiscovery`), on the same per-ticket lock and store lock every other write takes. It is an explicit action protocol, not an authentication system: `source` is a declared label attributing a write to a cooperating tool, recorded on the receipt and in the mutation log, and it does not authenticate a human or anything else.

**Precondition.** `ticket_show` reports `precondition` (`Ticket.Precondition()` in Go): an opaque token identifying the state the ticket was read at. It is compared exactly and nothing is derived from it — do not parse it, and do not assume it is a hash. An action carrying a precondition lands only if the ticket is still in that state; otherwise it is refused with `precondition conflict:` and the ticket is left unchanged, so the caller re-reads and decides again. An empty precondition makes no claim, and the action lands on whatever the ticket holds. The token covers the ticket's own file: an epic's status is derived from its children at read time, so a child reopening moves the epic's status without moving the epic's token.

**Receipt.** Every action records a receipt in the ticket file's `actions` block — `id`, `kind` (`apply` or `discover`), `digest`, `source`, `at`, `outcome` — alongside the note and the status change it made, in the same temp-file-and-rename write, so no partial action can persist: either the note, the transition and the receipt all landed or none did. The block is append-only (a write that drops or rewrites a receipt is refused; a file whose block cannot be read as written — a row missing a field, or a row count that does not match — is refused on read; and a file that carries receipts but cannot be read as a ticket, for that reason or for a fault in any other field, refuses every write over it, including a version-less `Update` of a rebuilt ticket, until the file is repaired by hand), replicates with the ticket through `tk sync`, and survives a restart of whatever wrote it. `outcome` is the status the ticket held after an apply, or the ID a discovery created. A moved ticket's copy carries no receipts: an action ID names the source ticket, and a finding key is scoped to the source project.

**Replay.** `action_id` is the caller's stable identity for one action. The same ID sent again with the same note and transition — after a lost response, a retry, or a restart — returns the recorded receipt with `replayed: true` and writes nothing, whatever the ticket's state or the precondition says by then: the recorded outcome is the answer. The same ID with different content is refused with `action conflict:`; a different action needs its own ID.

**Transitions.** The permitted set is `reopen` alone, which sets the status to `open` (clearing `completed`); on a ticket already open it is a no-op that still records its receipt, and on an epic it is refused, since an epic's status is derived from its children. Anything else is refused with `transition not permitted:`. Status changes outside that set stay `ticket_edit`'s.

**Discovery.** `ticket_discover` takes `ticket_create`'s destination (`project`, `repo` or the default project) and content arguments plus a required `finding_key`, scoped to the destination project — the same key in another project is a different finding. Under the exclusive store lock the project is scanned for a `discover` receipt under the key: found with the same content (title, description, design, acceptance, type, priority, parent and tags, digested exactly), that ticket comes back with `replayed: true` and nothing is created — concurrent and retried calls therefore yield one ticket, and every caller gets its ID; found with other content, the call is refused with `action conflict:` naming the existing ticket and nothing is created; found on more than one ticket, the call is refused with `action conflict:` naming every claimant and nothing is created (see Limits). This is exact-key deduplication: a finding that describes a problem an existing ticket already tracks in other words is a semantic duplicate, and searching for those (`ticket_search`) stays the caller's job. A project with a file that cannot be read as a ticket refuses discoveries until the file is repaired, since that file could be the one holding the key.

**Error prefixes.** The three refusals a caller has to tell apart carry stable prefixes on the error text: `precondition conflict:` (re-read the ticket and decide again), `action conflict:` (the ID or key was used for other content — a caller bug, not a retry), `transition not permitted:`. Every other failure is framed as `failed to apply action:` or `failed to create discovery:`. In Go the same three are `ticket.ErrConflict`, `ticket.ErrActionConflict` and `ticket.ErrTransitionNotPermitted`, matched with `errors.Is`.

**Limits.** The writer boundary is one machine: the per-ticket flock and the exclusive store lock serialise every tk process on a host, and the guarantees above are stated within it. Between machines the store is exchanged by git commits. Two machines applying one action ID to one ticket both append to that ticket's `actions` block, which surfaces as a merge conflict resolved by hand — the same statement the verdict ledger makes. Two machines discovering one finding key each create their own file, and separate files merge cleanly: the synced project then holds two tickets under the key, and `ticket_discover` refuses the key as ambiguous, naming both, rather than answering with either — an operator keeps one and removes the receipt from the other. A `Store` implementation outside the package cannot hold a lock across the replay check and the write, and both entry points refuse it rather than fall back to a retry loop that could apply an action twice. The protocol changes nothing about the ordinary tools: `ticket_edit`, `ticket_add_note` and `ticket_create` behave as before over a ticket carrying receipts, and the commit journal, links and retrospects are untouched.
...
- **Verify.** `ticket_verify` runs commands only in the checkout registered on this machine for the ticket's own project, or, with `dir`, in a linked git worktree of that checkout. A Root ticket (Root has no repository), a project with no checkout registered here, a registered checkout that is missing, and a `dir` outside that repository are each refused before anything runs, naming the reason, and nothing is recorded; the server's working directory, the store and a parent epic's project never stand in. `ticket_criteria` returns the parsed criteria and their `acceptance_id`; `ticket_verify` refuses a stale `acceptance_id` before running and does not record a run whose criteria changed underneath it. The report and the record carry the actual directory, the `acceptance_id` and the caller's `candidate`.
